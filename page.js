(function () {
	...
	const pageType = mStatus
		...
		: null;
	console.log(mStand);
	let pageCid = mStatus ? mStatus[1] : mStand ? mStand[2] : null;
	if (mFav && !pageCid) {
		...
		if (this._u.includes("/data/standings")) {
			this.addEventListener("load", () => {
				try {
					...
					console.error("[cfast] standings parse failed", e);
				}
			});
		}
		if (this._u.includes("/data/submitSource")) {
			try {
				...
				_open.call(
					this,
					"POST",
					`https://codeforces.com/data/submissionsDiff?${qs}`,
					this._a,
					this._b,
					this._c
				);
				...
				try {
					...
					if (pageType === "standings") {
						...
						let cell = document.querySelector(
							`td[acceptedsubmissionid="${sid}"], td[submissionid="${sid}"]`
						);
						if (!cell) {
							const linkEl = document.querySelector(
								`a[href*="/submission/${sid}"]`
							);
						...
						const row = cell
							? cell.closest("tr")
							: document
									.querySelector(`a[href*="/submission/${sid}"]`)
									?.closest("tr") ?? null;
						...
						let handle = "";
						if (row) {
							const ua = row.querySelector(
								'.contestant-cell a, a[href^="/profile/"]'
							);
							if (ua) {
						...
						let contestName = meta
							? meta.name
							: document
									.querySelector(".contest-name")
									.childNodes[1].textContent.trim();
						if (contestName && handle) {
							const url = `/submissions/${encodeURIComponent(
								handle
							)}/contest/${pageCid}`;
						...
						if (!problemName && cell && cell.cellIndex !== undefined) {
							const th =
								cell.closest("table")?.rows[0]?.cells[cell.cellIndex];
					...
					let href =
						(
							row.querySelector(`a.view-source[submissionid="${sid}"]`) ||
							{}
						).getAttribute("href") || "";
					if (
						!href.startsWith(`/contest/${effectiveCid}/submission/`) &&
						!href.startsWith(`/gym/${effectiveCid}/submission/`)
					) {
					...
					const partyName = ua
						? `<span title="${ua.title}" class="${
								ua.className
						  }">${ua.textContent.trim()}</span>`
						: "";
					const handle = ua ? ua.textContent.trim() : "";
					const ve =
						row.querySelector(".submissionVerdictWrapper>span") ||
						row.querySelector("td.status-verdict-cell span");
					const verdict = ve ? ve.outerHTML : "Compilation error";

					if (handle && meta?.name) {
						const url = `/submissions/${encodeURIComponent(
							handle
						)}/contest/${effectiveCid}`;
	...
	document.addEventListener("keydown", function (e) {
		if (
			...
			!/^\/contest\/\d+\/status/.test(location.pathname)
		)
			return;
		...
		if (e.shiftKey && (e.key === "c" || e.key === "C")) {
			...
			if (compareModeActive) {
				...
				const curRow = document.querySelector(
					`tr[data-submission-id=\"${currentSid}\"]`
				);
				if (curRow) {
		...
		if (e.key === "ArrowLeft") {
			...
			while (prev && !prev.hasAttribute("data-submission-id"))
				prev = prev.previousElementSibling;
			targetRow = prev;
		} else if (e.key === "ArrowRight") {
			let next = curRow.nextElementSibling;
			while (next && !next.hasAttribute("data-submission-id"))
	...
	function addCompareUI(force) {
		const header =
			document.querySelectorAll(".source-popup-header")[1] ||
			document.querySelectorAll(".source-popup-header")[0];
		if (!header) return;
		...
		if (pageType === "standings") {
			const cell =
				document.querySelector(
					`td[acceptedsubmissionid="${currentSid}"], td[submissionid="${currentSid}"]`
				) ||
				document
					.querySelector(`a[href*="/submission/${currentSid}"]`)
					?.closest("td");
    ...
    `;
		...
		compareBtn.addEventListener("click", () => {
			...
			fetch(
				...
				.then((j) => {
					const code =
						document.querySelectorAll(".source-popup-source")[1] ||
						document.querySelectorAll(".source-popup-source")[0];
